...
const App = () => {
  const [messages, setMessages] = useState([
    { role: 'ai', text: 'Gobernador, sistema Gamma en fase total. GestiÃ³n idiomÃ¡tica y monitores sincronizados. Â¿CuÃ¡l es su orden?' }
  ]);
  ...
  const handleExecute = async () => {
    ...
    try {
      ...
      if (data.candidates) {
        ...
        setMessages(prev => [...prev, { role: 'ai', text: `SISTEMA: Error en nÃºcleo. ${data.error?.message || ''}` }]);
      }
    } catch (error) {
      setMessages(prev => [...prev, { role: 'ai', text: 'ERROR DE FASE: Interferencia en la lÃ­nea.' }]);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleNodeAction = (node, action) => {
    ...
    switch(action) {
      ...
      case 'HISTORY':
        ...
        msg = `MEMORIA: Extrayendo histÃ³rico de maniobras de ${node.id}.`;
        break;
      ...
      case 'UNDO':
        ...
        msg = `RESTAURACIÃ“N: Preparado comando para deshacer cambios en ROOT.`;
        break;
    }
  ...
  return (
    <div className="h-screen bg-[#020617] text-slate-200 font-sans p-2 md:p-4 flex flex-col overflow-hidden">
      ...
      <div className="flex flex-1 gap-2 overflow-hidden">
        {/* COLUMNA 1: GESTIÃ“N IDIOMÃTICA */}
        ...
        <main className="flex-[2] flex flex-col bg-slate-900/40 rounded-3xl border border-white/5 overflow-hidden shadow-inner">
          ...
          <div className="p-4 bg-black/40 border-t border-white/5">
            <div className="flex gap-2 bg-slate-950 p-2 rounded-2xl border border-white/10 focus-within:border-purple-500/50 transition-all overflow-hidden shadow-2xl">
              <textarea 
                placeholder="Escriba su orden estratÃ©gica (Ctrl+Enter para enviar)..."
        ...
        <aside className="hidden xl:flex flex-1 flex flex-col gap-2">
          <div className="flex-1 bg-slate-900/60 rounded-3xl border border-white/5 overflow-hidden flex flex-col">
            <div className="p-2 bg-green-500/10 border-b border-white/5 flex justify-between px-4"><span className="text-[9px] font-black text-green-400 uppercase">PRODUCCIÃ“N</span><Eye size={10}/></div>
          ...
          <div className="flex-1 bg-slate-900/60 rounded-3xl border border-white/5 overflow-hidden flex flex-col">
            ...
            <iframe key={`root-${refreshKey}`} src="https://dev.via51.org" className="w-full h-full border-none opacity-80 hover:opacity-100 transition-all"/>
          </div>
        </aside>
      </div>
    </div>
